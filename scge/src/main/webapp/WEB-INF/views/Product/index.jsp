            ...
            <table border="0" cellpadding="4" cellspacing="0">
              ...
              <c:forEach var="product" items="${productList.pageList}">
                <tr>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${fn:trim(product.gender) eq fn:trim('m')}">
                                Activo
                            </c:when>
                            <c:when test="${fn:trim(product.gender) eq fn:trim('f')}">
                                Desactivado
                            </c:when>
                        </c:choose>
                    </td>
                    <td><fmt:formatDate dateStyle="full" pattern="dd-MM-yyyy" value="${product.dob}"/></td>
                    <td>
                        <a href="view.html?productId=${product.id}" class="view">ver</a>
                        <a href="update.html?productId=${product.id}" class="update">actualizar</a>
                        <a href="delete.html?productId=${product.id}" class="delete">eliminar</a>
                    </td>
                </tr>
              </c:forEach>
              <c:if test="${productList.nrOfElements == 0}">
